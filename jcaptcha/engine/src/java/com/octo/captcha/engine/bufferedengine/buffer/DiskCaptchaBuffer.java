...
public class DiskCaptchaBuffer implements CaptchaBuffer
{
    ...
    /**
     * If persistent, the disk file will be kept and reused on next startup. In addition the memory
     * store will flush all contents to spool, and spool will flush all to disk.
     */
     ...
     *                  like c:/temp/name
     * @param persistant
     *                  If the disk buffer is persistant, it will try to load from file name .data et
     *                  .index existing data
     */
    ...
    /**
     * Shuts down the disk store in preparation for cache shutdown <p/>If a VM crash happens, the
     * shutdown hook will not run. The data file and the index file will be out of synchronisation.
     * At initialisation we always delete the index file after we have read the elements, so that it
     * has a zero length. On a dirty restart, it still will have and the data file will
    ...
    /**
     * Writes the Index to disk on shutdown <p/>The index consists of the elements Map and the
     * freeSpace List <p/>Note that the cache is locked for the entire time that the index is being
     * written
     */
    ...
    /**
     * Reads Index to disk on startup. <p/>if the index file does not exist, it creates a new one.
     * <p/>Note that the cache is locked for the entire time that the index is being written
     */
    ...
    /**
     * The design of the layout on the data file means that there will be small gaps created when
     * DiskElements are reused.
     * 
     * @return the sparseness, measured as the percentage of space in the Data File not used for
     *             holding data
     */
    ...
    /**
     * When elements are deleted, spaces are left in the file. These spaces are tracked and are
     * reused when new elements need to be written. <p/>This method indicates the actual size used
     * for data, excluding holes. It can be compared with {@link #getDataFileSize()}as a measure of
     * fragmentation.
     */
    ...
    /**
     * @return the file name of the data file where the disk store stores data, without any path
     *             information.
     */
    ...
    /**
     * @return the file name of the index file, which maintains a record of elements and their
     *             addresses on the data file, without any path information.
     */
